...
def update_certificate(
    client, old_certificate_id, pubkey, privkey, tag, resource_types
):
    ...
    params = {
        ...
        "Tags": [{"TagKey": "ssl-tag", "TagValue": tag}],
    }
...
def find_certificate_id(client, tag):
    ...
    params = {"Tags": [{"TagKey": "ssl-tag", "TagValue": tag}], "Limit": 1}
...
def main():
    parser = argparse.ArgumentParser(
        description="Update old certificate in tencent cloud ssl"
    )
    parser.add_argument(
        "oldCertificateTag", type=str, help="the certificate you want to update"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        default=["teo", "cdn", "clb", "vpc", "waf"],
        help="resource types to update the certificate for",
    )
